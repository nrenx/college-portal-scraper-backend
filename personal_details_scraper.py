            ...
            while retries <= max_retries:
                try:
                    ...
                    if retries > max_retries:
                        ...
                        raise
                    logger.warning(f"Network error in {func.__name__}: {e}. Retrying ({retries}/{max_retries})...")
    ...
    def select_form_filters(self, academic_year: str, year_of_study: str, branch: str, section: str) -> Optional[BeautifulSoup]:
        ...
        try:
            ...
            if self.driver:
                ...
                logger.info(f"Available academic year options:")
                for i, option in enumerate(academic_year_select.options):
                    option_text = option.text.strip()
                    option_value = option.get_attribute('value')
                    logger.info(f"  Option {i}: text='{option_text}', value='{option_value}'")

                # CRITICAL FIX: For the personal details page, we need to explicitly select the option
                # that matches the academic year, not just the first one that contains it

                # Map academic years to specific indices in the dropdown
                # This is a direct approach based on the observation that the options are in a specific order
                academic_year_indices = {
                    "2022-23": 17,  # Option 17 in the dropdown (based on logs)
                    "2023-24": 18,  # Option 18 in the dropdown (based on logs)
                    "2024-25": 19,  # Option 19 in the dropdown (based on logs)
                    "2021-22": 16,  # Option 16 in the dropdown (based on logs)
                    "2020-21": 15,  # Option 15 in the dropdown (based on logs)
                    "2019-20": 14   # Option 14 in the dropdown (based on logs)
                }

                # CRITICAL FIX: Always try to use the index-based approach first
                try:
                    # First approach: Use our predefined indices based on logs
                    if academic_year in academic_year_indices:
                        index = academic_year_indices[academic_year]
                        if index < len(academic_year_select.options):
                            # Use JavaScript to ensure the selection works
                            self.driver.execute_script(
                                "arguments[0].selectedIndex = arguments[1]; arguments[0].dispatchEvent(new Event('change'));",
                                academic_year_select._el,
                                index
                            )
                            logger.info(f"Selected academic year by predefined index {index} using JavaScript")

                            # Double-check with regular selection
                            try:
                                academic_year_select.select_by_index(index)
                                logger.info(f"Confirmed academic year selection by index {index}: {academic_year_select.first_selected_option.text}")
                            except Exception as e:
                                logger.warning(f"Could not confirm selection with regular method: {e}")
                        else:
                            logger.warning(f"Index {index} is out of range for academic year options (max: {len(academic_year_select.options)-1})")
                            # Fallback to direct selection
                            academic_year_select.select_by_visible_text(academic_year)
                            logger.info(f"Selected academic year by visible text: {academic_year}")
                    else:
                        # Fallback to direct selection
                        academic_year_select.select_by_visible_text(academic_year)
                        logger.info(f"Selected academic year by visible text: {academic_year}")
                except Exception as e:
                    logger.warning(f"Error selecting academic year: {e}, trying alternative methods")
                    try:
                        # Try to find an option with text exactly matching the academic year
                        option_found = False
                        for option in academic_year_select.options:
                            option_text = option.text.strip()
                            if academic_year == option_text:
                                academic_year_select.select_by_visible_text(option_text)
                                option_found = True
                                logger.info(f"Selected academic year by exact text match: {option_text}")
                                break

                        if not option_found:
                            # If no match found, select the first option
                            academic_year_select.select_by_index(0)
                            logger.warning(f"Could not find academic year '{academic_year}', selected first option: {academic_year_select.first_selected_option.text}")
                    except Exception as e2:
                        logger.error(f"Failed to select academic year: {e2}")

                time.sleep(1.0)  # Increased wait time for the page to update

                # Verify the selected academic year
                selected_option = academic_year_select.first_selected_option
                selected_text = selected_option.text.strip()
                selected_value = selected_option.get_attribute('value')
                logger.info(f"Verified selected academic year: text='{selected_text}', value='{selected_value}'")

                # Check if the selected academic year matches what we requested
                if academic_year not in selected_text:
                    logger.warning(f"WARNING: Selected academic year '{selected_text}' does not match requested academic year '{academic_year}'")

                    # CRITICAL FIX: Force the correct academic year selection
                    try:
                        # Find the correct option index
                        correct_index = academic_year_indices.get(academic_year, 0)

                        # Use JavaScript to set the selection - this is the most reliable method
                        self.driver.execute_script(
                            "arguments[0].selectedIndex = arguments[1]; arguments[0].dispatchEvent(new Event('change'));",
                            academic_year_select._el,
                            correct_index
                        ...
                        selected_text = selected_option.text.strip()
                        selected_value = selected_option.get_attribute('value')
                        logger.info(f"After JavaScript intervention: text='{selected_text}', value='{selected_value}'")

                        # If still not correct, try one more approach - direct selection by text
                        if academic_year not in selected_text:
                            logger.warning(f"JavaScript intervention failed, trying direct selection by text")

                            # Find the option with the exact text
                            for i, option in enumerate(academic_year_select.options):
                                if academic_year == option.text.strip():
                                    # Use JavaScript to set the selection
                                    self.driver.execute_script(
                                        "arguments[0].selectedIndex = arguments[1]; arguments[0].dispatchEvent(new Event('change'));",
                                        academic_year_select._el,
                                        i
                                    )
                                    logger.info(f"Selected academic year by exact text match using JavaScript: {option.text.strip()}")

                                    # Verify again
                                    selected_option = academic_year_select.first_selected_option
                                    selected_text = selected_option.text.strip()
                                    selected_value = selected_option.get_attribute('value')
                                    logger.info(f"After final JavaScript intervention: text='{selected_text}', value='{selected_value}'")
                                    break
                    except Exception as e:
                        logger.error(f"Error trying to set academic year using JavaScript: {e}")

                # CRITICAL: If we're still not selecting the correct academic year, log a critical error
                selected_option = academic_year_select.first_selected_option
                selected_text = selected_option.text.strip()
                if academic_year not in selected_text:
                    logger.critical(f"CRITICAL ERROR: Failed to select academic year '{academic_year}', still showing '{selected_text}'")
                    logger.critical(f"This will cause the scraper to scrape the wrong academic year data!")
...
def main():
    ...
    # Start with a clear message about the script's purpose
    logger.info("="*80)
    logger.info("STARTING PERSONAL DETAILS SCRAPER")
    logger.info("="*80)
    ...
    # Log the academic year being processed
    if args.academic_year:
        logger.info(f"Processing ONLY academic year: {args.academic_year}")
    else:
        logger.info("No specific academic year specified, will process all years")
    ...
    if args.academic_year and args.year_of_study and args.branch and args.section:
        ...
        if args.academic_year:
            # If academic year is specified, use ONLY that academic year
            academic_years = [args.academic_year]
            logger.info(f"Using only the specified academic year: {args.academic_year}")
        else:
            # Otherwise use all academic years
        ...
        # CRITICAL FIX: Override the academic year in all combinations with the requested academic year
        # This ensures we always use the academic year specified in the command line arguments
        if args.academic_year:
            logger.info(f"CRITICAL FIX: Overriding all combinations to use the requested academic year: {args.academic_year}")
            combinations = [(args.academic_year, year_of_study, branch, section)
                           for _, year_of_study, branch, section in combinations]
            logger.info(f"Modified {len(combinations)} combinations to use academic year {args.academic_year}")
    ...
    if num_workers > 1:
        ...
        if total_students > 0:
            ...
            # Still consider this a success since the script ran without errors
            logger.info("\nPersonal details scraping completed with no data found.")

        # Print a clear success message that the backend can detect
        logger.info("="*80)
        logger.info("PERSONAL DETAILS SCRAPING COMPLETED SUCCESSFULLY")
        logger.info("="*80)

        # Always return success since the script ran without errors
        # This prevents the backend from marking it as failed
        return 0


if __name__ == "__main__":
    # Call main and exit with success code (0)
    main()
    sys.exit(0)
